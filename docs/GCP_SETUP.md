...
- Access to the GCP project (e.g., `rle-ruritania`)
...
```bash
gcloud projects add-iam-policy-binding "rle-ruritania" \
  --member="serviceAccount:github-actions-rle@rle-ruritania.iam.gserviceaccount.com" \
...
```bash
gcloud projects describe rle-ruritania --format="value(projectNumber)"
```
...
gcloud iam service-accounts add-iam-policy-binding \
  "github-actions-rle@rle-ruritania.iam.gserviceaccount.com" \
  --project="rle-ruritania" \
...
| `GCP_SERVICE_ACCOUNT` | `github-actions-rle@rle-ruritania.iam.gserviceaccount.com` |
...
```bash
python scripts/rasterize_ecosystem_to_cog.py --project rle-ruritania
```
...
- **enables the `storage.googleapis.com` API** and **creates a public-read bucket** `gs://rle-ruritania-rle-cogs` if it does not already exist;
...
```
Caller does not have required permission to use project rle-ruritania.
